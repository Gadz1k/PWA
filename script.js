...
const visibilty = document.querySelector('span.visibility');
const pollutionImg = document.querySelector('p.pollution_img');
const pollutionValue = document.querySelector('span.pollution_value');

const apiInfo = {
    link: 'https://api.openweathermap.org/data/2.5/weather?q=',
    key: '&appid=24bd426b5e8d54e3fdb86e14d23397ab',
    units: '&units=metric',
    lang: '&lang=pl',
}
...
function getWeatherInfo() {
    ...
    axios.get(URL).then((response) => {
        ...
        [cityName, temp, description, feelsLike, pressure, humidity, visibilty, clouds, windSpeed].forEach(e1 => {
            e1.textContent = '';
        })
        weatherImg.src = '';
    }).finally(() => {
